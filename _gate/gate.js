...
const BUY = {
  checkout : 'https://muslimkidschecklist.gumroad.com/l/sgpcb',   // Family Access — $40/yr
  upgrade  : 'https://muslimkidschecklist.gumroad.com/l/mtdviu',  // Second App Access — $25/yr
  bundle   : 'https://muslimkidschecklist.gumroad.com/l/bkvfvr',  // All Three Apps — $89/yr
...
/* ---------------------------------------------------------------
   ...
   So this list IS the entitlement rule. Rebuilt 20 Aug 2026: each app
   in the family (Spelling Quest, One Ayah At A Time, Muslim Kids
   Checklist) now has its OWN three Gumroad products — Family Access,
   Second App Access, and All Three Apps — instead of one shared
   discount code and a reused bundle ID. This app accepts a key issued
   for any of ITS OWN three products.

   Still to add here: the other two apps' own "All Three Apps" bundle
   product IDs, once Spelling Quest and One Ayah publish theirs — a
   bundle key bought from either sibling app must also unlock this one.
   Not yet received from that project as of 20 Aug.

   Find each id in Gumroad: Products → the product → Content tab →
   Insert → License key block (a base64-looking string, NOT the
...
const PRODUCTS = [
  ...
  // { id: '', label: 'One Ayah At A Time — All Three Apps' } // <- add once known
];
...
function screenExpired(dead){
  ...
  <p class="gSmall" style="text-align:center">Already have ${BUY.siblings}? Add this one for just ${BUY.upgPrice}.</p>
  ${buyButton(`Add for ${BUY.upgPrice}/year`,'upgrade')}
  <hr class="gDiv">
